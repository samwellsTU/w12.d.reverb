...
outputGain.gain.value = 1; // Set output volume to 50% -6dBFS
...
const createReverb = async function () {
  let myConvolver = audioContext.createConvolver();

  //load impulse response
  let response = await fetch("ir/chopin.wav");
  let arraybuffer = await response.arrayBuffer();
  myConvolver.buffer = await audioContext.decodeAudioData(arraybuffer);
  return myConvolver;
};
